 ...
 * UCI 著法轉中文記譜。
 *
 * 自 `poc/index.html` 的 `uci2cn` 移植。移植時逐條對規則驗過而非照單全收,並在
 * `tests/test_web_pure.py` 補上針對性案例(同線雙車、同線雙馬、同線兵、進退平)。
 * 結論是 POC 的判別與規則一致,行為照搬,只補註解與命名。
 *
 * 移植當下曾以為「前/後」分支從未被實戰跑過,後由 5.2 的端到端實測推翻:起始
 * FEN 的 d1/d3 就是一對黑卒共線,實戰中亦出現 g6/g8 雙傌(記譜「前傌進五」)。
 * 該分支確實會被走到 —— 這反而說明,靠一局棋恰好走到的路徑當覆蓋率並不可靠。
 *
