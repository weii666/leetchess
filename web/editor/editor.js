 ...
 *
 * `checkForm()` 回傳的每一項都定位到欄位,因此主要的呈現就在那一欄底下的訊息槽
 * (8.4)。寫入操作旁邊那一行則是同一份清單的**點名匯總**,理由有二:被停用的按鈕
 * 說不出自己為什麼按不下去;而 FEN 欄位空著時那一欄刻意不掛訊息(2.5),少了匯總
 * 就會有一項未通過完全不出現在畫面上。兩處都由同一次 `render()` 寫入,不會各說各話。
 *
 * ## 盤面唯讀,靠的是空的著法集合
 *
 ...
 *
 * **FEN 那一欄的訊息不取自 `checkForm()`**,而是 FEN 的當下呈現(`readFenView`)。
 * 差別只在一種輸入:空的 FEN 欄位這裡什麼都不說,而 `checkForm()` 對它有一項
 * 「請填入 FEN」。理由是 requirement 2.5 —— 清空 FEN 呈現的是空盤面而不是解析失敗,
 * 一個還沒開始填的欄位不該掛著錯誤訊息。那一項未通過並沒有因此消失:寫入照樣停用,
 * 而「是哪一項」由 `renderWriteAction()` 的點名補上(8.4)。
 *
...
function renderMessages(issues, fenMessage) {
  ...
  for (const issue of issues) {
    if (issue.field !== null && texts.get(issue.field) === '') {
