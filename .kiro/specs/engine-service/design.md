...
- 對局 session 狀態 —— 由前端持有。**產品不提供悔棋**(見 `product.md`),故無悔棋記憶,亦不需要單次挑戰內的應手穩定性
...
實作階段實測推翻了一個原本的假設。POC 註記「200k 與 2M 得同一手」,原設計據此認為算力需求極低 —— 該註記**只對著法成立,對 mate 分數不成立**。

以《適情雅趣》第 21 局,每次用全新進程(冷雜湊表)量測:

| 節點數 | 起始局面(紅方視角) | 走 f8f9 後(黑方視角) | 耗時 |
|---|---|---|---|
| 200k | `mate 17`(高估) | `cp 526`(未搜到殺) | 0.10s |
| 230k | — | `mate -16` | 0.11s |
| **250k** | **`mate 16`(正確)** | **`mate -16`** | **0.12s** |
| 1M | `mate 16` | `mate -15`(精確) | 0.55s |
| 2M | `mate 16` | `mate -15` | 1.18s |

**bestmove 在所有節點數下完全相同**(`f8f9` / `e9f9`)—— 黑方應手的品質確實不受影響,POC 那句話的正確部分在此。

**決策:採 250k。** 這是取得 mate **型別**的門檻,低於此三態信號會大量落在「未知」,使 Requirement 2 形同虛設。

**已知限制:殺著倒數(DTM)可能高估 1 步。** 取得精確 DTM 需 1M,代價是每手 0.55 秒而非 0.12 秒 —— 相同承載人數下池容量與 CPU 需求皆為 4.5 倍。倒數屬諮詢性質,不值得付這個成本。**UI 呈現殺著倒數時宜表述為「約 N 步」而非確數。**
