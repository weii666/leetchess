...
- 4.1:**著法驗證器刻意拋 `InvalidMoveFormatError` 而非 `ValueError`** —— pydantic-core 只攔截 `ValueError`/`AssertionError`,自訂例外會穿透 body 驗證並可由已註冊的處理器捕捉。review 以最小 app 實測:**有處理器 → 400 帶 `INVALID_MOVE_FORMAT`;無處理器 → 500**。故 4.2/4.3 必須註冊處理器,且路由程式不得吞掉它(`except ValidationError` **抓不到**它)。
