...
> **後續更新:R5.8(排版與既有題目檔一致)已移除。** 以下發現當時用來支撐「`tags` 需手刻成單行」的自建序列化器,但維護成本(手刻邏輯 + 整組回歸測試)與它防到的問題不成比例,人工討論後決定拿掉;`serializePosition` 現在對所有欄位一律呼叫 `JSON.stringify`。本節保留作為歷史紀錄。
...
- **Context**:5.7 要求寫入後既有的每一題逐字不變。(原本 5.8 也要求排版與既有檔一致,現已移除,但不影響本決策 —— 方案 2 的選擇單靠 5.7 就成立。)
- **Alternatives Considered**:
...
- **Follow-up**:無(原本的「序列化器以既有題庫檔做回歸比對」隨 5.8 一併移除)。
...
| JSON 排版 | ~~自建序列化器~~ **(5.8 已移除)** | 改回統一呼叫 `JSON.stringify`,不再手刻特例 |
