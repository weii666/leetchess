...
- [x] 3.2 (P) 題目序列化與文字層追加
  ...
  - 完成狀態:三種形態各有對應輸出;非陣列輸入拋出專屬錯誤;追加後的輸出可驗證為以既有全文為前綴
  - _Requirements: 5.4, 5.5, 5.6, 5.7, 5.9, 7.4, 7.5_
  - _Boundary: corpus-file.js_
  - **後續更新**:原本要求「標籤陣列維持單行」且與既有題庫檔逐字比對(5.8)已移除,`tags` 改回交給通用 `JSON.stringify` 處理,不再手刻
...
- [ ] 7.4 (P) corpus-file.js:序列化略過不存在的欄位
  ...
  - 完成狀態:不帶 `description` 的候選題目輸出五個欄位,其餘欄位的排版與有描述時完全相同
