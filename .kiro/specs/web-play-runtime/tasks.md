...
- 2.2:**POC 的 `uci2cn` 經獨立驗證後判定正確**,無須修正。但值得記:**POC 的前/後分支在《適情雅趣》第 21 局裡一次都沒被執行過**(該局全程沒有同名子共線)—— 「POC 跑過整局」對這段邏輯完全不構成證據。現在有 12 個測試蓋著,review 另枚舉雙相/雙仕/雙傌的所有合法目的地確認無記譜碰撞。
