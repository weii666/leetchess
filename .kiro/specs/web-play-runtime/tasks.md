...
**已延後的項目**(見 requirements 的 `## Backlog`):6.3 的取消等待中請求(單次搜尋僅 0.12 秒,且前端取消不會中止後端搜尋)。

**本輪只用到兩個後端端點**:載入題目與取得對手應手。局面查詢端點沒有使用場景 —— 載入題目的回應已含起始局面的合法著法,重來時再取一次即可。
...
- 4.4:**「數半手」的等待規則對最後一手不夠**(修正 4.3 的 note)。`game.play()` 在第一個 `await` 之前就同步把使用者的半手推進序列並 `notify()`,所以 `click()` 回來時計數已經是 1;而 `move: null` 時計數**永遠不再變**。正確的沉澱信號當時定為**等 `#waiting` 轉隱藏** —— `waiting` 在點擊派發內同步翻成 true,無競態。**該元素其後整條移除**(它在版面流裡來去會推動底下的歷史著法,requirement 6.1 已降級改寫),信號改為**等 `#turn` 不再說「(某方)走棋」**:輪方同樣在點擊派發內同步翻成對手,無競態的性質原樣保留。定義見 `tests/test_web_play.py` 的 `SETTLED`。
