...
- **重新開啟後的狀態重建、取消等待中的請求** —— 已列 requirements 的 `## Backlog`。範圍原則:進入題目後不做比 POC 明顯複雜的功能
- 任何象棋規則的自行判斷
...
fen / notation  →  board / api  →  game  →  app
                                   catalog  ↗   (problem-browser 4.6,見下)
```

> **`app.js` 另外依賴 `web/catalog.js`** —— 那是 problem-browser 的純資料模組(取題庫索引、判斷可上架),為其 requirement 4.6「獲勝後前往下一題」而加。它不碰 DOM、不依賴任何 web 模組,對 `app.js` 而言與 `fen.js` 同層,故依賴方向仍然往左。本 spec 的模組本身沒有多出任何相依。
...
若呈現層各自持有狀態,這兩條路徑會各自需要清理邏輯,而清理不乾淨正是「畫面卡在不可知狀態」的來源(requirements 7.4)。

### 每手棋一次請求

使用者走出一手後**直接呼叫黑方應手端點**,不先查詢局面狀態。該回應已涵蓋「紅方這一手就將死黑方」(對手著法為空、對局結束、勝方為紅),而那是每一題排局的最後一手,不是邊緣案例。

多一次呼叫會讓通過後端併發閘門的次數加倍,直接壓縮服務可承載人數。

**本輪只用到兩個端點。** 狀態重建延後之後,`POST /api/state` 沒有使用場景 —— 載入題目的 `GET /api/positions/{id}` 回應已含起始局面的合法著法,前端在載入時就取得並留住起始狀態。**重來刻意不打後端** —— 它是失敗後的復原路徑,若自己依賴網路,斷線時使用者會被鎖死,反而違反 7.4。契約仍完整支援該端點,只是前端不呼叫。

### 為何不做取消(Backlog 6.3)

單次搜尋約 0.12 秒加 RTT,使用者按下取消之前多半已經回來了。且前端取消**不會中止後端搜尋** —— 它仍會跑完並佔用池容量,省不下任何資源。

若日後節點數調高(例如為了精確 DTM 改用 1M,每手 0.55 秒)使等待變得明顯,再行考慮;屆時若要真正中止,得先在後端契約加入取消機制。
...
├── api.js           # 後端 client:請求、逾時、兩種錯誤形狀的辨識
├── game.js          # 對局狀態機:走法序列、推進、重來
└── app.js           # 組裝與事件綁定
```
...
├── test_web_play.py      # 互動:走子、推進、終局、重來
└── test_web_failures.py  # 失敗路徑:逾時、斷線、忙碌、序列不一致
```
...
| 4.4 | 走脫回饋來源可擴充 | game.js |
| 5.1, 5.2 | 重來、不提供單手回退 | game.js |
| 6.1, 6.2, 6.4 | 等待呈現、等待中不接受走子、等待態必解除 | app.js, game.js |
| 7.1, 7.2, 7.3 | 逾時、忙碌、序列不一致 | api.js, app.js |
| 7.4 | 失敗後維持可操作 | game.js |
| 7.5 | 不呈現原始錯誤,含無法辨識的格式 | api.js |
...
| api.js | 後端 client 與錯誤辨識 | 1.4, 3.1, 3.5, 7.1–7.5 | 無 |
| game.js | 對局狀態機 | 2.4, 2.5, 3.x, 4.3, 4.4, 5.1, 5.2, 6.2, 6.4, 7.4 | api.js |
| app.js | 組裝、事件綁定、呈現 | 1.2–1.4, 3.2, 4.1–4.2, 8.3–8.4 | game.js, board.js, notation.js |

### api.js

**責任**:唯一與後端往來的地方。每次請求帶逾時上界,並把後端的兩種回應形狀收斂成統一的失敗表示。

**關鍵約束**:

- 後端錯誤有**兩種形狀** —— 端點錯誤為 `{code, message}`(七種碼之一),路由層 404/405 為框架原生的 `{"detail": ...}`。任何無法辨識的形狀一律歸為通用錯誤,**不得把原始內容呈現給使用者**(7.5)
- 每次請求都要有逾時上界,逾時與斷線都轉為可辨識的失敗(7.1)
- **不支援取消**(Backlog 6.3)—— 見上方「為何不做取消」
...
- 走脫回饋的來源須可擴充:目前只有 `signal`,日後判定表是**新增來源**而非改寫推進流程(4.4)
- 任何失敗之後狀態必須回到可操作(7.4),等待態必被解除(6.4)
- **不做狀態重建**(Backlog 5.3/5.4)—— 走法序列沒有持久化來源,進度持久化屬 problem-browser
...
| 題目不存在(404) | 找不到題目 | —— |
| 其他已知錯誤碼 | 通用錯誤訊息 | 重試 |
| 無法辨識的回應 | 通用錯誤訊息(7.5) | 重試 |

**共同保證**:任何失敗之後介面維持可操作(7.4)、等待態解除(6.4)、不呈現後端原始內容或技術細節(7.5)。錯誤呈現以**單一通用區塊**為主,不為每種錯誤各做一套 UI —— 區分只需到「可重試」與「須重來」兩類。
...
- 逾時、斷線、503、409、404 的呈現與可復原性(區分到「可重試」與「須重來」兩類即可)
- **無法辨識的回應形狀**(模擬路由層的 `{"detail": ...}`)歸為通用錯誤,不呈現原始內容(7.5)
