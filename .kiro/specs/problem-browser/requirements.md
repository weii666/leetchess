...
2. When 列表呈現, the 題庫瀏覽介面 shall 對每一題顯示題號、局名、難度與標籤。出處**不在列表呈現**,改由對局介面顯示(見 4.5);描述**兩頁皆不呈現** —— 列表是掃視用的,每列擠進越多欄位越難掃。
3. The 題庫瀏覽介面 shall 使題號與局名成為每一列的主要識別,可供快速掃視。題號**只呈現數字本身**,不加「第…題」之類的贅字。
...
### Requirement 2: 篩選 —— **Backlog(本輪不實作)**

> **本輪不實作介面。** 純資料層(`web/catalog.js` 的 `filterPositions()`)已完成並經突變驗證,本需求的全部 AC 在資料層皆已滿足;缺的只是介面與互動。觸發條件:題庫規模成長到光靠捲動找不到題目為止 —— **目前題庫只有 1 題,篩選沒有意義**。詳見 tasks.md 的 Backlog 段落。
...
5. When 對局介面開啟, the 對局介面 shall 顯示該題的出處(已自列表移來,見 1.2),並與最長殺著距離**併為單行**。**描述兩頁皆不呈現。**
