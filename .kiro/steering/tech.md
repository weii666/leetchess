...
跳步、重來都只是重送一次。服務端不持有任何一局的進度 —— 對局狀態由前端持有,這也是後端能以多進程水平擴展的前提。
...
- **黑方在最頑強一檔內變化** — 不追求 bit 級一致。變化發生在**跨次重玩**之間,這正是要的效果:消掉背題目。**單次挑戰內不需要任何穩定性保證** —— 產品不提供悔棋(見 `product.md`),同一局面不會被重複請求
