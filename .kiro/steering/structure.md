...
| `description` | 字串 | **否** | 完整描述,允許換行空格,涵蓋書名、局號、局名。**選填**:產品的兩個畫面都不渲染它(`web/list.js` 刻意不取用、`web/app.js` 的 `#puzzle-description` 已移除),而它的內容就是「出處 + 局號 + 局名」的串接,與 `title` 重複。既有題目一律帶著它,缺席時讀為空字串 |
...
- **出處資料夾**:繁體中文書名,分卷者以 `~` 綴在書名後,如 `適情雅趣~卷一`。**分隔符是 `~` 而非 `-`**,與檔名的局號區間(`20-24.json`)用的 `-` 區隔開,免得「書名-卷次」與「起局號-迄局號」看起來是同一種寫法。此處原本誤記為 `-`,磁碟自 `dc9e30b` 起即為 `~`
  - **書名與卷次是兩件事**:資料夾名帶卷次,但題目的 `description` 只寫書名(如「適情雅趣 第二五局 患在几席」)。要由資料夾名取得書名,須切掉 `~` 之後的卷次
